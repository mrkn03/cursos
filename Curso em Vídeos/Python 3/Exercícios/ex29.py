#29. Escreva um programa que faça o computador 'pensar' em um numero inteiro entre 0 e 5 e peça para o
#usuario tentar descobrir qual foi o numero escolhido pelo computador. O programa devera escrever na tela
#se o usuario venceu ou perdeu.
import random
numero = int(input('Escolha um numero de 0 a 5:'))
n = random.randint(0,5)

if n == numero:
    print('Parabens!, você ganhou')

else:
    print('Você perdeu')

print('Você escolheu o numero {} e o computador escolheu o numero {}'.format(numero,n))
